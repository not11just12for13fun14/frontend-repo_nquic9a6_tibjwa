...
function RequestForm({ selectedProvider, selectedCategory, onSubmitted }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
      <div className="font-semibold text-slate-900">Request a {selectedCategory} service {selectedProvider ? `with ${selectedProvider.name}` : ''}</div>
      ...
      <textarea className="input min-h-[110px]" placeholder="Describe the job" value={form.description} onChange={(e)=>update('description', e.target.value)} required />
      <div className="flex items-center gap-3">
        <button disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg disabled:opacity-60 shadow-sm">{loading ? 'Submitting…' : 'Submit Request'}</button>
